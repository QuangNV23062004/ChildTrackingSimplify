...
export default function DoctorRequestsPage() {
  ...
  const handleAction = async (id: string, status: "Doctor_Accepted" | "Doctor_Rejected") => {
    setActionLoading(id + status);
    try {
      await api.patch(`/Request/${id}/status`, { status });
      toast.showToast(`Request ${status === "Doctor_Accepted" ? "approved" : "rejected"}!`, "success");
      fetchRequests();
    } catch {
              ...
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-gray-50 transition-colors duration-200">
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-gray-900">{req.member?.name || "N/A"}</div>
                    <div className="text-xs text-gray-500">{req.member?.email}</div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex gap-2 justify-center">
                      <button
                        ...
                        onClick={() => handleAction(req.id, "Doctor_Accepted")}
                        disabled={actionLoading === req.id + "Doctor_Accepted"}
                      >
                        {actionLoading === req.id + "Doctor_Accepted" ? "Approving..." : "Approve"}
                      </button>
                      <button
                        className="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition cursor-pointer disabled:opacity-50"
                        onClick={() => handleAction(req.id, "Doctor_Rejected")}
                        disabled={actionLoading === req.id + "Doctor_Rejected"}
                      >
                        {actionLoading === req.id + "Doctor_Rejected" ? "Rejecting..." : "Reject"}
                      </button>
                    </div>
                  </td>
                </tr>
