...
export default function UserConsultationsPage() {
  ...
  const [forumConsultation, setForumConsultation] = useState<Consultation | null>(null);
  const [endModalOpen, setEndModalOpen] = useState(false);
  const [endingConsultation, setEndingConsultation] = useState<Consultation | null>(null);
  ...
  useEffect(() => {
    const fetchConsultations = async () => {
      ...
      setLoading(true);
      try {
        const response = await consultationService.getConsultationsByMemberId(userId);
  ...
  const handleEndConsultation = async (consultation: Consultation) => {
    ...
    if (!confirmed) return;
    try {
      await consultationService.updateConsultationStatus(consultation.id, "Completed");
  ...
  if (loading) {
    return <Loading message="Loading your consultations..." className="h-screen" />;
  }

  return (
    <div className="p-4 sm:p-6 bg-white">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Your Consultations</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border border-gray-200 rounded-lg">
              <thead className="bg-gray-100 text-xs uppercase text-gray-600">
                <tr>
                  <th className="px-4 py-3 text-left font-semibold tracking-wider">Consultation Info</th>
                  ...
                  <th className="px-4 py-3 text-center font-semibold tracking-wider">Actions</th>
                </tr>
              </thead>
                ...
                {consultations.map((consultation) => {
                  ...
                  <tr key={consultation.id} className="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full text-black ${getStatusColor(consultation.status)}`}>
